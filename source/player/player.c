...
void ship_gamemode(Player *player) {
    ...
    if (state.input.holdJump) player->vel_y += grav(player, ship_accel) * STEPS_DT;
    else player->vel_y -= grav(player, ship_accel) * STEPS_DT;
...
void ufo_gamemode(Player *player) {
    if (state.input.pressedJump) set_p_velocity(player, 550.0f, false);
    player->rotation = (player->vel_y / 15.0f);
}

void wave_gamemode(Player *player) {
    float wave_speed = 600.0f;
    if (state.input.holdJump) player->vel_y = grav(player, wave_speed);
    else player->vel_y = grav(player, -wave_speed);
...
void robot_gamemode(Player *player) {
    ...
    if (player->on_ground) player->rotation = 0;
}
...
void draw_robot(Player *player, float x, float y, float scale, u32 col1, u32 col2) {
    ...
    C2D_DrawSprite(get_sprite(robot_part_body_idx), x, y, p_rot, scale, col1, col2, flip);
    C2D_DrawSprite(get_sprite(robot_part_head_idx), x, y, p_rot, scale, col1, col2, flip);
    C2D_DrawSprite(get_sprite(robot_part_front_leg_idx), x, y, p_rot - leg_rot, scale, col1, col2, flip);
    */
...
void draw_player(Player *player) {
    ...
    if (player->gamemode == GAMEMODE_ROBOT) draw_robot(player, x, y, scale, c1, c2);
    else spawn_icon_at(player->gamemode, selected_cube, true, x, y, player->lerp_rotation, (state.mirror_mult < 0), player->upside_down, scale, c1, c2, 0xFFFFFFFF);
}
